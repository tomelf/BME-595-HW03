...
class NeuralNetwork(object):
    ...
    def build(self, in_layer, *h_arr):
        for i in range(len(h_arr)):
            ...
            e = h_arr[i]
            th = torch.normal(means=torch.zeros(e, s+1), std=torch.zeros(e, s+1).fill_(1/math.sqrt(e))).type(torch.FloatTensor)
    ...
    def forward(self, input):
        ...
        bias = torch.ones(1, x.size()[1]).type(torch.FloatTensor)
        ...
        for idx, th in enumerate(self.Theta):
            ...
            bias = torch.ones(1, self.a[idx].size()[1]).type(torch.FloatTensor)
    ...
    def backward(self, target):
        ...
        bias = torch.zeros(1, target.size()[1]).type(torch.FloatTensor)
